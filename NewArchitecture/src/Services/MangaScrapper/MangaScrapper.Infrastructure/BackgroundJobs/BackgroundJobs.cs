    ...
    ILogger<ChapterScrapingJob> logger)
{
    ...
    public async Task ExecuteAsync(Guid mangaId, string mangaTitle, double chapterNumber, string chapterId, string serviceType, CancellationToken ct)
    {
        ...
        var scopedRepo = scope.ServiceProvider.GetRequiredService<IScrapperRepository>();

        var manga = await scopedRepo.GetDocumentByIdAsync(mangaId, ct);
        if (manga == null)
        ...
        logger.LogInformation("Finished scraping for {MangaTitle} - Chapter {ChapterNumber}. Pages saved: {PageCount}",
            mangaTitle, chapterNumber, processedChapter.Pages.Count);
    }
}
    ...
    Microsoft.Extensions.Options.IOptions<Configuration.ScrapperSettings> settings)
{
    ...
    public async Task ExecuteAsync(Guid mangaId, string mangaTitle, CancellationToken ct)
    {
        ...
        var manga = await repo.GetDocumentByIdAsync(mangaId, ct);
        if (manga == null)
        {
        ...
        var domainRepo = scope2.ServiceProvider.GetRequiredService<Domain.Repositories.IMangaRepository>();
        await domainRepo.DeleteAsync(Domain.ValueObjects.MangaId.From(mangaId), ct);
